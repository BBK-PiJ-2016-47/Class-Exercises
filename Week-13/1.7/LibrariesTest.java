...
public class LibrariesTest {
	...
	public void checkCounts() {
		int books = newLib.getBookCount();
		int borrowed = newLib.getBookBorrowedCount();
		int users = newLib.getReaderCount();
		assertEquals(0, books);
		assertEquals(0, borrowed);
		assertEquals(0, users);
	}
	
	@Test
	public void checkCounts_Incremented() {
		newLib.addBook("Title", "author");
		int books = newLib.getBookCount();
		assertEquals(1, books);
		newLib.takeBook("Title");
		int borrowed = newLib.getBookBorrowedCount();
		assertEquals(1, borrowed);
	}
